...
- **Theme sync**: the greeter reads the same `ThemeEngine`/`Theme` singletons as the main shell, but selection is persisted with `Quickshell.statePath(...)` (per-config under `~/.local/state/quickshell/by-shell/<hash>/`, one JSON file — see `common/state/Preferences.qml`). The greeter is a separate config root and usually runs as its own system user, so it does not see the session's saved theme/font on its own and falls back to the first entry in `themes.json` / the engine default font. The launcher's **Sync theme to greeter** action (`shell/scripts/sync-greeter-preferences.sh`, run via `pkexec`) copies the current theme/font into the greeter's `preferences.json` one-shot — it computes that file's path itself as `MD5(absolute path to /etc/quickshell/greeter/shell.qml)` (Quickshell's own `statePath` hashing, verified against `src/launch/command.cpp`/`launch.cpp` in Quickshell's source) rather than asking a running greeter process, so it works even though the greeter isn't running during a normal desktop session. Not a live/automatic sync — re-run the action after changing the theme if you want the login screen to match.
